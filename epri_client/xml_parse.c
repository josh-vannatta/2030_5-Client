...
// Skip an unknown XML element whose opening tag has already been peeked.
// Returns 1 on success, 0 if data is incomplete.
static int xml_skip_unknown (Parser *p) {
  if (p->token == EMPTY_TAG) {
    p->need_token = 1;
    return 1;
  }
  // START_TAG: consume content until the matching END_TAG
  int depth = 1;
  p->need_token = 1;
  while (depth > 0) {
    switch (xml_token (p->xml)) {
...
int xml_next (Parser *p) {
  ...
  while (p->state == PARSE_NEXT) {
    if (!se->n) {
      // End of remaining schema elements. If an XML element is pending,
      // search the full sequence for an out-of-order or unknown element.
      if (!p->empty && p->stack.n > 0) {
        switch (parse_token (p)) {
        case XML_INCOMPLETE: return 0;
        case XML_INVALID: p->state = PARSE_INVALID; break;
        case START_TAG: case EMPTY_TAG: {
          // Search the full sequence from its beginning
          StackItem *t = stack_top (&p->stack);
          const SchemaElement *seq =
            &p->schema->elements[t->se->index + 1];
          const SchemaElement *found = NULL;
          for (const SchemaElement *s = seq; s->n; s++) {
            if (!s->attribute &&
                streq (se_name (s, p->schema), p->xml->name)) {
              found = s; break;
            }
          }
          if (found) {
            start_tag (p, found); // consume token, set p->empty
            p->state = PARSE_ELEMENT;
            se = found; // se++ below → found+1; p->se = found ✓
            break;
          }
          // Unknown element: skip it and continue looking
          if (!xml_skip_unknown (p)) return 0;
          continue; // skip se++ to stay at n==0 and get next token
        }
        default:
          p->state = PARSE_END;
        }
	...
	p->state = PARSE_ELEMENT;
      ...
      else if (p->token == START_TAG || p->token == EMPTY_TAG) { se++; continue; }
